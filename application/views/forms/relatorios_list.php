        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                $this->table->set_heading('ID', 'Clientes Masters ', 'Departamentos', 'Colaboradores ', 'Locais Visitados',
                    'Data', 'check in', 'check out', ' Quilômetros rodados ', 'Valor reembolsado', 'KM Value', 'Req. Just', 'Justificativa');
                                        ...
                                        foreach ($departament['employees'] as $employee) {
                                            if ($employee['locals']) {
                                                foreach ($employee['locals'] as $local) {
                                                    $valor = @$local['km_travalled'];

                                                    $new = number_format($valor, 2, ',', ' ');
                                                    $reembolso = (@$local['km_travalled']) * @$departament['value'];
                                                    $this->table->add_row(
                                                        ['data' => @$local['id']],
                                                        ['data' => @$row['name']],
                                            ...
                                            if ($employee['agendas']) {
                                                foreach ($employee['agendas'] as $agendas) {
                                                    $valor = @$agendas['routes']['km_travalled'];
                                                    //                                                    print_r($valor);
                                                    $idagenda = $agendas['id'];
                                                    ...
                                                    $reembolso = @$agendas['km_travalled'] * @$departament['value'];
                                                    $new = number_format($valor, 2, ',', ' ');
                                                    $this->table->add_row(
                                                        ['data' => @$agendas['id']],
                                                        ['data' => @$row['name']],
                                                    ...
                                                    <script>
                                                        $(document).ready(function () {
                                                            var id = '<?php echo $idagenda?>';
                                                            $('#requisitar_<?php echo $idagenda?>').bind('click', function () {
                                                                var value = true;
...
//                                        $reembolso = (@$local['km_traveled'] / 1000) * @$departament['value'];
                                        $valor = @$newrow['km_travalled'];

                                        $new = number_format($valor, 2, ',', ' ');
                                        $reembolso = (@$newrow['km_travalled']) * @$row['departament_value'];
                                        $this->table->add_row(
                                            ['data' => @$row['name']],
                                                            ...
                                                            <div class="row">
                                                                <div class="col-lg-12">
                                                                    <div class="form-group">
                                                                        ...
                                                                        echo form_textarea(
                                                                            'employee',
                                                                            set_value('employee', @$newrow['need_justification']),
                                                                            'class="form-control employee" disabled'
                                                                        );
